...
//	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, DISABLE);
	...
	else if (((ADCResult>=50)&&(ADCResult<=70)))
		{
			rot_switch_mode=2;//{printf("BW Mode: 1| ");}
		}
	else if (((ADCResult>=100)&&(ADCResult<=125)))
		{
			rot_switch_mode=3;//{printf("BW Mode: 1| ");}
		}
	else if (((ADCResult>=155)&&(ADCResult<=185)))
		{
			rot_switch_mode=4;//{printf("BW Mode: 1| ");}
		}
	else if (((ADCResult>=200)&&(ADCResult<=240)))
		{
			rot_switch_mode=5;//{printf("BW Mode: 1| ");}
		}
	else if (ADCResult>=241)
		{
...
void Display_MODE(void)
{
	...
	switch(rot_switch_mode)
	{
		case 1:
				{
					lcd_gotoxy(0,2);
//					sprintf(lcd,"ADC_Result: %d",(int)ADCResult);lcd_putstr(lcd);
...
//					lcd_gotoxy(0,2);
//					sprintf(lcd,"ADC_Result: %d",(int)ADCResult);lcd_putstr(lcd);
					sprintf(lcd,"BW FIREFIGHT");lcd_putstr(lcd);
				}break;
		case 3:
				{
					lcd_gotoxy(2,2);
//					lcd_gotoxy(0,2);
//					sprintf(lcd,"ADC_Result: %d",(int)ADCResult);lcd_putstr(lcd);
					sprintf(lcd,"NAV. MENU");lcd_putstr(lcd);
				}break;
		case 4:
				{
					lcd_gotoxy(0,2);
